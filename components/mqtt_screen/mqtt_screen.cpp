...
namespace esphome
{
    namespace mqtt_screen
    {
        ...
        static const char *const TAG = "mqtt_screen";
        ...
        void MqttScreen::dump_config()
        {
            ESP_LOGCONFIG(TAG, "  Topic: %s", this->topic_.c_str());
        }

        void MqttScreen::process(int32_t status_code, uint32_t duration_ms)
        {
            ESP_LOGI("mqtt_screen", "Process HTTP Response with SC: %d", status_code);
            ...
            // Update Display
            ESP_LOGI("mqtt_screen", "Update Screen");
        ...
        void MqttScreen::setup()
        {
            mqtt_client_->subscribe(
                topic_,
                std::bind(&MqttScreen::on_message, this, std::placeholders::_1, std::placeholders::_2));

            display_->set_writer(
                std::bind(&MqttScreen::draw, this, std::placeholders::_1));
            ...
            auto lambdaaction = new LambdaAction<int32_t, uint32_t>(
                std::bind(&MqttScreen::process, this, std::placeholders::_1, std::placeholders::_2));
            automation->add_actions({lambdaaction});
        }

        void MqttScreen::on_message(const std::string &topic, const std::string &message)
        {
            ESP_LOGI("mqtt_screen", "Received MQTT URL %s", message.c_str());
            ...
            http_client_->set_url(message);

            ESP_LOGI("mqtt_screen", "Request Payload from HTTP");
            http_client_->send({trigger_});
            ESP_LOGI("mqtt_screen", "Finished HTTP");
        }

        void MqttScreen::draw(display::Display &display)
        {
            display.fill(display::COLOR_OFF);
            ESP_LOGI("mqtt_screen", "Begin drawing");
            ...
            ESP_LOGI("mqtt_screen", "Finished drawing");
        }

        float MqttScreen::get_setup_priority() const
        {
